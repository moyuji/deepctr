...
class SDM(BaseTower):
    def __init__(self, user_dnn_feature_columns, item_dnn_feature_columns, gamma=1, dnn_use_bn=True,
                 ...
                 dnn_dropout=0, init_std=0.0001, seed=1024, task='binary', device='cpu', gpus=None, output_dim=1024, dense_dim=30,
                 item_norm_weight_start=0.0, user_norm_weight_start=0.0, item_norm_weight_end=0.0, user_norm_weight_end=0.0,
                 norm_weight_warmup=1, gating_warmup=5, beta=0.1, momentum=0.1, logger=None, nd_sample=False,sigma=5.0):
        ...
        if len(user_dnn_feature_columns) > 0:
            self.user_dnn = SparseDenseEncoding(compute_input_dim(user_dnn_feature_columns), dnn_hidden_units, beta=beta,
                                           activation=dnn_activation, l2_reg=l2_reg_dnn, dropout_rate=dnn_dropout,
                                           use_bn=dnn_use_bn, init_std=init_std, device=device, output_dim=output_dim,
                                           norm_weight=user_norm_weight_start, momentum=momentum, nd_sample=nd_sample, dense_dim=dense_dim)
            self.user_dnn_embedding = None

        if len(item_dnn_feature_columns) > 0:
            self.item_dnn = SparseDenseEncoding(compute_input_dim(item_dnn_feature_columns), dnn_hidden_units, beta=beta,
                                           activation=dnn_activation, l2_reg=l2_reg_dnn, dropout_rate=dnn_dropout,
                                           use_bn=dnn_use_bn, init_std=init_std, device=device, output_dim=output_dim,
                                           norm_weight=item_norm_weight_start, momentum=momentum, nd_sample=nd_sample, dense_dim=dense_dim)
